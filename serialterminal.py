...
parser.add_argument('-p', '--port', type=str, default='', required=True, help='give port to listen from, eg "/dev/cu.wchusbserial1410" or "1"')
...
# port can be given as /dev/cu.wchusbserial1440 or 4
print("waiting for serial...")
waiting_for_port = True
while waiting_for_port:
    for p in list_ports.comports():
        if re.search(r"^\d$", args.port) and re.search(r"wchusbserial.*?{port}0$".format(port=args.port), p.device):
            args.port = p.device
